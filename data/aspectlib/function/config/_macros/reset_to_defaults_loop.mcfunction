...
$data modify storage aspectlib:cache temp.config set from storage aspectlib:registry config[$(registry_config_list)]

# Set Default
function aspectlib:config/_macros/set_defaults with storage aspectlib:cache temp.config

# Reduce list count
scoreboard players remove #aspects aspectlib.temp 1
# Loop if list count is 0 or above, otherwise return success
## Return
execute unless score #aspects aspectlib.temp matches 0.. run return 1
## Recursion
