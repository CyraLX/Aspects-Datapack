...
execute unless score #aspects aspectlib.temp matches ..2 run return fail
...
$data modify storage aspectlib:users player.$(id).aspects.aspect set from storage aspectlib:registry aspect[{namespace: "$(aspect_namespace)", name: "$(aspect_name)"}]
# Migrate aspect data to namespaced version
$execute unless data storage aspectlib:registry aspect[{namespace: "$(aspect_namespace)", name: "$(aspect_name)"}] run return fail
