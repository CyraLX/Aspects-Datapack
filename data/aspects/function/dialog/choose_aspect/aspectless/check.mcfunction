...
execute if entity @p[tag=get_debug_logs] unless score @s aspects.dialog.menu_requested matches 1 if score #aspects aspects.config.force_default_aspect matches 1.. run tellraw @a[tag=get_debug_logs] {text:"",color:"#aaaaaa",extra:[{text:"",color:"#f88379",extra:["[",{translate:"aspects"},"]: "]},{selector:"@s",color:"gold"}," had a ", {text:"Default Aspect",color:"gold"}, " assigned to them"]}
execute unless score @s aspects.dialog.menu_requested matches 1 if score #aspects aspects.config.force_default_aspect matches 1.. run return run function aspects:aspect/human/activate
