# Set score id of new Aspect to user
...
$data modify storage aspectlib:users player.$(id).aspects.aspect set from storage aspectlib:registry aspect[{namespace:"$(new_aspect_namespace)", name: "$(new_aspect_name)"}]
...
execute unless score #aspects aspects.config.world.track_statistics matches 1.. run return fail
...
$execute if data storage aspectlib:registry aspect[{namespace:"$(old_aspect_namespace)", name: "$(old_aspect_name)"}] store result score #aspects.old aspects.aspect_id run data get storage aspectlib:registry aspect[{namespace:"$(old_aspect_namespace)", name: "$(old_aspect_name)"}].id
$execute store result score #aspects.new aspects.aspect_id run data get storage aspectlib:registry aspect[{namespace:"$(new_aspect_namespace)", name: "$(new_aspect_name)"}].id
...
# Stats debug logs
# $function aspects:stats/aspect/get with storage aspectlib:registry aspect[{namespace:"$(old_aspect_namespace)", name: "$(old_aspect_name)"}]
# $function aspects:stats/aspect/get with storage aspectlib:registry aspect[{namespace:"$(new_aspect_namespace)", name: "$(new_aspect_name)"}]