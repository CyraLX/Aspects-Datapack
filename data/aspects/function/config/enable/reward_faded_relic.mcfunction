...
execute if entity @p[tag=get_debug_logs] unless score #aspects aspects.config.reward_faded_relic matches 1 run tellraw @a[tag=get_debug_logs, tag=!config_changer] [{"text":"[Aspects]: ","color":"#f88379"},{"selector":"@s","color":"#aaaaaa","extra":[{"text":" set "},{"text":"REWARD_FADED_RELIC","color":"gold"},{"text":" to "},{"text":"TRUE","color":"gold"}]}]
