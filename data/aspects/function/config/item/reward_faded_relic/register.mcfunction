...
#                           Defaults to [""]
#
data modify storage aspectlib:dummy config set value {namespace:"aspects",type:"item",name:"reward_faded_relic",dialog_type:1,description_with:[{translate:"options.on",color:"green"}]}
# Define Default value
scoreboard players set #aspects aspectlib.dummy 1

# Convert legacy scoreboard
## From V2 to V3
execute if score #aspects aspects.config.reward_faded_relic = #aspects aspects.config.reward_faded_relic run scoreboard players operation #aspects aspects.config.item.reward_faded_relic = #aspects aspects.config.reward_faded_relic
