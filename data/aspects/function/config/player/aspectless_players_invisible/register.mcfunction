...
#                           Defaults to [""]
#
data modify storage aspectlib:cache temp.config set value {namespace:"aspects",type:"player",name:"aspectless_players_invisible",dialog_type:1,description_with:[{translate:"options.on",color:"green"}]}
# Define Default value
scoreboard players set #aspectlib aspectlib.temp 1

# Convert legacy scoreboard
## From V2 to V3
execute if score #aspects aspects.config.aspectless_players_invisible = #aspects aspects.config.aspectless_players_invisible run scoreboard players operation #aspects aspects.config.player.aspectless_players_invisible = #aspects aspects.config.aspectless_players_invisible
