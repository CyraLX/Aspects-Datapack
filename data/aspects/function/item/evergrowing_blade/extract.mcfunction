...
execute if entity @s[type=!minecraft:player, tag=!smithed.entity] unless entity @p[tag=aspects.evergrowing_blade.attacker,distance=..32,predicate=aspects:item/essence_cage_collector/is_held] if predicate aspects:aspect/any run return run function aspects:item/evergrowing_blade/awaken_from_aspect_item
