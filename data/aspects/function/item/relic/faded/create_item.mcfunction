...
execute store result storage aspectlib:cache temp.random_1 short 1 run random value 1..11 aspects:item/relic/faded
execute store result storage aspectlib:cache temp.random_2 short 1 run random value 1..4 aspects:item/relic/faded
execute as @n[type=minecraft:armor_stand, tag=aspects_item_granter, distance=..16] run function aspects:item/relic/faded/apply_random_lore with storage aspectlib:cache temp
