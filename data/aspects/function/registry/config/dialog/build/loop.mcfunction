...
$data modify storage aspectlib:cache temp.config_builder.config set from storage aspects:registry config[$(registry_config_list)]

# Add actions to body
function aspects:registry/config/dialog/build/action with storage aspectlib:cache temp.config_builder.config
...
execute if score #aspects aspectlib.dummy > #aspects aspects.registry.config.list_index run return run function aspects:registry/config/dialog/build/dialog with storage aspectlib:cache temp.config_builder
## Recursion
